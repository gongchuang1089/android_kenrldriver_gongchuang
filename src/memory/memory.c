...
int pte_process_memory_rw_cached(enum request_op op, pid_t pid, uint64_t vaddr, void *buffer, size_t size)
{
    ...
    while (bytes_remaining > 0)
    {
        ...
        else
        {
            ...
            */
            ...
            if (current_vaddr >= task_size || bytes_this_page > task_size - current_vaddr)
            {
                status = -EFAULT;
                s_last_vpage_base = -1ULL;
                if (op == REQ_PTE_PHYS_READ_MEMORY && size > 8) __builtin_memset((uint8_t *)buffer + bytes_copied, 0, bytes_this_page);
                goto next_chunk;
            }
        ...
        {
            s_last_vpage_base = -1ULL;
            if (op == REQ_PTE_PHYS_WRITE_MEMORY && size > 8) __builtin_memset((uint8_t *)buffer + bytes_copied, 0, bytes_this_page);
            goto next_chunk;
        }
